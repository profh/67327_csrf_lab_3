...
class DB
  class << self
    ...
    def create_products(session_id)
      ...
      create_dragon_chess_set(session_id)
    end
    
    def create_dragon_chess_set(session_id)
      dragon = Product.new(:upc => 123123123123, :name => 'Green Dragon Chess Set', :image => nil, :description => 'A beautiful board and set of pieces involving dragons. Perfect set for the Sicilian Dragon player.', :price => 497.97, :is_active => true)
      dragon.session_id = session_id
      dragon.save!
    end
    ...
    def create_users(session_id)
      ...
      user = User.new(username:'mheimann', password:'qwerty', first_name:'Mark', last_name:'Heimann', race:'asian', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      ...
      user = User.new(username:'lheimann', password:'john316', first_name:'Trying to hack me?', last_name:'You are in trouble!', race:'white', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      ...
      # another user for for potential brute force
      user = User.new(username:'raja', password:'letmein', first_name:'Raja', last_name:'Sooriamurthi', race:'asian', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      user = User.new(username:'jquesenberry', password:'abc123', first_name:'Jeria', last_name:'Quesenberry', race:'white', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      user = User.new(username:'egruberman', password:'password', first_name:'Ed', last_name:'Gruberman', race:'white', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      user = User.new(username:'tgruberman', password:'123456', first_name:'Ted', last_name:'Gruberman', race:'white', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
